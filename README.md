...
- **Analytics** — a single read-only `AnalyticsEngine` over the booking set: booked minutes, revenue, average duration, bookings by date / day-of-week / hour, busiest day, peak hour, cancellation rate, booking horizon, a customer leaderboard (with an optional minimum-bookings floor), per-resource and per-active-staff utilisation, and a day-by-day utilisation report with ASCII bars. Its value types live in `model/Metrics.kt` so reports and the CLI share them.
...
│   ├── AnalyticsEngine.kt        # Read-only aggregate metrics, leaderboards and utilisation
│   ├── BookingQueries.kt         # Shared confirmed/cancelled booking query helpers
